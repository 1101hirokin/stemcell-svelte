...
親 490px(px 閾値 480 の直上)・実物の Button 3個(日本語ラベル)で観測:
...
部品の中身は健全なまま。行が部品から溢れる(scrollWidth 超過)ことは両者ともなかった
...
- px 側: 「同じ幅なら同じ形」がユーザーの文字設定に依存しなくなる(500px の部品は誰の環境でも同じ形)。
