 ...
 *   2. min の値は常用の器の幅でどこまで区別に効くか(語彙を段にするなら何段が意味を持つか)。
 ...
 * 器の幅を走査する。jsdom はレイアウトを計算しないため実ブラウザを使う。
 */
...
/** 折れ点: folded → 横 に変わる最小の器の幅を二分探索で求める。 */
...
console.log('   期待: 列数 = floor((W + gap) / (256 + gap))。器 < min では1列が器幅へ縮む(min(…,100%) の防波堤)');
...
console.log('== 2. min の値 × 常用の器の幅: 列数の行列(語彙の解像度の材料) ==');
const widths = [320, 360, 600, 768, 1024, 1280, 1536];
console.log(`  器の幅(px): ${widths.join(' / ')}`);
...
console.log('== 4. 文字拡大(root 16/20/24px)× min=16rem, 器 1024px ==');
...
}
{
  ...
  console.log(`  器 500px(折れ域): folded=${narrow.folded}, 脇の幅=${narrow.sideW}px(全幅=行の独占)`);
  const wide = await sidebarShape(900, '15rem', '50%');
  console.log(`  器 900px(横域): folded=${wide.folded}, 脇=${wide.sideW}px / 本体=${wide.contentW}px`);
}

console.log('== 6. Sidebar: viewport からの独立(同じ器の幅なら viewport が違っても同じ形) ==');
for (const vp of [600, 1400]) {
  await page.setViewportSize({ width: vp, height: 800 });
  const r = await sidebarShape(700, '15rem', '50%');
  console.log(`  viewport=${vp}px, 器=700px: folded=${r.folded}`);
}
