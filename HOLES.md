...
| 18 | (仕様側) | tokensRequired の欠落: TextField.css は shape.border-width と focus-ring の幾何(width / style / offset)を使うが、契約の tokensRequired に無い。同じ欠落は Button(shape.border-width)にもあり、Divider / Card は宣言している。適合テストはこれらの存在を検査できない | 実装はトークンを使う(発明ではない。値は tokens が持つ)。契約側の宣言追加は版バンプを伴う仕様修正として申し送る | A・既知(仕様へ戻す対象。独立レビュー minor 指摘) |
