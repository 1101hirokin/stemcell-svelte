...
| 24 | Button | form 内での type の既定。仕様はフォーム参加(type=submit 相当)を未決のまま置く(Button.md §6 の TODO)が、実装が native の button を素のまま出す以上、HTML の既定により form 内では submit として振る舞う(独立レビューが実測で検出: form に入れて click すると submit イベントが発火する) | native 既定を維持し、AGENTS.md が注意喚起する。type="button" 固定は Enter の暗黙送信まで変えるため、仕様の裁定(Button.md §6)を待つ。実測材料として仕様側へ報告 | B・要裁定 |
