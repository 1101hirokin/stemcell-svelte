...
| 15 | TextField | value の FW 写像。契約は value + change(逐次)で、uncontrolled は「土地の便宜であり契約外」(field.md §5) | `$bindable` にして bind:value を許す(Svelte の土地の便宜)。bind しない消費者には controlled + onchange がそのまま成立する | B(HOLES #4 と同型) |
