<script lang="ts">
  ...
  // ここには何も無い、という報せ(EmptyState.md)。空は正常な状態で、器が持つのは何が無いのかを言葉に
  // することと、次にできることを置く場所だけである。失敗は扱わない(空は正常、失敗は異常)。
  // 0 件になったことの告知も持たない(器は自分が現れたことしか知らない。patterns/empty-results.md)。
  interface Props {
...
<!-- 意味を運ぶのは見出しと説明で、絵は装飾として支援技術から外す。器は role を立てず、生きた領域も
