...
const aside = createRawSnippet(() => ({ render: () => '<nav>脇</nav>' }));
...
it('side は DOM 順を決める: start = 脇→本体、end = 本体→脇(視覚だけの入替をしない。WCAG 1.3.2)', () => {
  const { container } = render(Sidebar, { props: { aside, children: kids } });
  let order = [...root(container).children].map((el) => el.className);
  expect(order).toEqual(['sc-sidebar-side', 'sc-sidebar-content']);
  const { container: c2 } = render(Sidebar, { props: { aside, children: kids, side: 'end' } });
...
it('視覚だけの入替(order / row-reverse)を使っていない', () => {
  const { container } = render(Sidebar, { props: { aside, children: kids, side: 'end' } });
...
it('contentMin: 既定 50%。百分率でない文字列と 100% 超は warn して 50% へ退避する', () => {
  ...
  for (const bad of ['banana', '50', '30rem', '101%', '']) {
    const { container: c2 } = render(Sidebar, {
      props: { aside, children: kids, contentMin: bad },
    });
    expect(
  ...
  const { container: c3 } = render(Sidebar, {
    props: { aside, children: kids, contentMin: '60%' },
  });
...
it('gap: 段は data-gap、原始 X はインライン style、語彙外は warn して md(layout.md §6)', () => {
  const { container } = render(Sidebar, { props: { aside, children: kids, gap: 'lg' } });
  expect(root(container).dataset.gap).toBe('lg');
  const { container: c2 } = render(Sidebar, { props: { aside, children: kids, gap: '16' } });
  expect(root(c2).style.gap).toBe('var(--spacing-16)');
  const warn = vi.spyOn(console, 'warn').mockImplementation(() => {});
  const { container: c3 } = render(Sidebar, { props: { aside, children: kids, gap: 'huge' } });
