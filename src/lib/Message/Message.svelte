<script lang="ts">
  import './Message.css';
  import { META, type MessageRole } from './meta';
  ...
  let { role = META.props.role.default, speakerLabel, speaker, meta, children }: Props = $props();
</script>

<!-- 発話は一つのまとまりとして届く(group)。名前は話者から作る(誰の発話かを視覚だけに頼らない。
     conversation §5)。器自身は焦点を受けず、中の押せる要素に焦点と当たり判定が生きる。 -->
<div class="sc-message" data-role={role} role="group" aria-label={speakerLabel}>
