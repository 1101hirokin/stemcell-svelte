<script lang="ts">
  ...
  // turn はその列に対する描画時の view である。部品はその view を一つ描くだけで、中身は解釈しない
  // (part の集合は閉じている。part ごとの描画を部品が持つと data のパススルーを受け取れなくなる)。
  interface Props {
    /** この発話の話者(conversation §2 の閉集合)。帰属であって見た目ではない。 */
    role?: MessageRole;
    /** 面の量(emphasis.md §3 の4段)。部品は role から姿を導かないので、変えたい消費者が渡す。 */
...
<!-- 発話は一つのまとまりとして届く(group)。名前は話者から作る(誰の発話かを視覚だけに頼らない。
     conversation §5)。部品自身は焦点を受けず、中の押せる要素に焦点と当たり判定が生きる。
