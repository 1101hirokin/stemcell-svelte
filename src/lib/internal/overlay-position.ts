...
export function resolveSides(
  ...
  const block = anchor.top + anchor.height / 2 < room.height / 2 ? 'end' : 'start';
  const wantedLeft = align === 'center' ? centerX - contentWidth / 2 : anchor.left;
  const wantedFits =
    wantedLeft >= OVERLAY_EDGE && wantedLeft + contentWidth <= room.width - OVERLAY_EDGE;
  const inline = wantedFits ? align : centerX < room.width / 2 ? 'start' : 'end';
  return { block, inline };
}
...
export function inlineShift(left: number, width: number): number {
  ...
  if (left < OVERLAY_EDGE) return OVERLAY_EDGE - left;
  if (left + width > room - OVERLAY_EDGE) return room - OVERLAY_EDGE - (left + width);
  return 0;
}
...
export function availableBlockSize(
  ...
  return block === 'end'
    ? room - (anchor.top + anchor.height) - gap - OVERLAY_EDGE
    : anchor.top - gap - OVERLAY_EDGE;
}
